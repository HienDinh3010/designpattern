...
public class Application {
	...
	public static void main(String[] args) {
		...
		Predicate<Customer> customerFromChicagoPredicate = c -> c.getAddress().getCity().equals("Chicago");
		Iterator<Customer> filterIterator = customerCollection.filterIterator(customerFromChicagoPredicate);
		while (filterIterator.hasNext()) {
			System.out.println(filterIterator.next());
		}

		System.out.println("Customers whose zip code starts with 12");
		Predicate<Customer> customerWhoseZipCodeStartsWith12 = c -> c.getAddress().getZip().startsWith("12");
		Iterator<Customer> filterIterator2 = customerCollection.filterIterator(customerWhoseZipCodeStartsWith12);
